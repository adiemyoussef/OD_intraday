...
def get_file_from_sftp(msg_body):
    logger.debug("Entered verify_and_process_message")
...
def fetch_poly_data(previous_date, current_date, previous_datetime, current_datetime):
    ...
    logging.info(f'Fetched {len(poly_data)} in {time.time() - start_time} sec.')
    return poly_data
...
def prepare_poly_fetch(current_time=None):
    ...
    if is_next_day_period:
        logger.debug("We are in the next day period")
        ...
        logger.debug("We are NOT in the next day period")
        current_date = current_time.date()
            ...
            if file_info:
                ...
                if df is not None and not df.empty:
                    ...
                    for attempt in range(MAX_ACK_RETRIES):
                        try:
                            rabbitmq_utils.safe_ack(message_frame.delivery_tag,message)
