...
from utilities.sftp_utils_async import *
from utilities.db_utils_async import *
from utilities.rabbitmq_utils_async import *
...
async def process_last_message(rabbitmq_utils:AsyncRabbitMQUtilities):
    ...
    try:
        ...
        expected_file_name = 'Cboe_OpenClose_2024-07-24_15_40_1.csv.zip'
...
async def update_book_with_latest_greeks(book: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # breakpoint()
    poly_data_list = await db_utils.execute_query(test_query)
...
async def process_intraday_data():
    ...
    logger.info(f"[process_intraday_data] db status: {await db_utils.get_status()}")
    try:
