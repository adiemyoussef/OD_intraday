...
async def process_last_message_in_queue(rabbitmq_utils:RabbitMQUtilities):
    prefect_logger = get_run_logger()
    prefect_logger.info("process_last_message_in_queue")
...
def fetch_poly_data(previous_date, current_date, previous_datetime, current_datetime):
    ...
    """
    return db_utils.execute_query(query)
...
def filter_and_log_nan_values(final_book):
    ...
    logger.info(f"Filtered out {len(nan_rows)} rows with NaN values")
    logger.info(f"Unique contracts filtered: {nan_contracts}")
    logger.info(f"MM positions of filtered contracts: \n{nan_rows.groupby('option_symbol')['mm_posn'].sum()}")
...
def update_book_with_latest_greeks(book: pd.DataFrame, poly_data: pd.DataFrame) -> pd.DataFrame:
    ...
    poly_data['strike_price'] = poly_data['strike_price'].astype(int)
    poly_data['contract_id'] = poly_data['option_symbol'] + '_' + \
                          poly_data['call_put_flag'] + '_' + \
                          poly_data['strike_price'].astype(str) + '_' + \
                          pd.to_datetime(poly_data['expiration_date_original']).dt.strftime('%Y-%m-%d')
    ...
    logger.info("Number of rows with NaN values in greeks: {}".format(false_count))
...
def prepare_poly_fetch(current_time=None):
    ...
    # Determine if we're in the next day period (after 8:20 PM)
    ...
    if is_next_day_period:
        ...
        current_date = next_business_days[0].date()
    else:
    ...
    logger.info(f"Prepared poly fetch parameters: "
...
def ensure_connections():
    ...
    while attempt < max_attempts:
        ...
        if db_status['status'] == 'Connected' and rabbitmq_status['status'] == 'Connected':
            prefect_logger.info("Both database and RabbitMQ connections are established.")
            return True
...
def Intraday_Flow():
    ...
    try:
        ...
        prefect_logger.info(f"[process_intraday_data] db status: {db_utils.get_status()}")
        prefect_logger.info(f"[process_intraday_data] rabbitmq status: {rabbitmq_utils.get_status()}")
        ...
        try:
            ...
            if file_info:
                ...
                if df is not None and not df.empty:
                    ...
                    logger.info(f"Len of final_book: {len(final_book)}")

                    # Filter out NaN values and log
                    filtered_final_book = filter_and_log_nan_values(final_book)
                    logger.info(f"Len of filtered_final_book: {len(final_book)}")
                    breakpoint()
                    ...
                    for attempt in range(MAX_ACK_RETRIES):
                        try:
                            rabbitmq_utils.safe_ack(message.delivery_tag)
                            logger.info(f"Message acknowledged for file: {file_info['file_name']}")
                            break
                        except Exception as e:
                            logger.error(f"Error acknowledging message (attempt {attempt + 1}/{MAX_ACK_RETRIES}): {e}")
                    ...
                    rabbitmq_utils.safe_nack(message.delivery_tag, requeue=True)
            else:
