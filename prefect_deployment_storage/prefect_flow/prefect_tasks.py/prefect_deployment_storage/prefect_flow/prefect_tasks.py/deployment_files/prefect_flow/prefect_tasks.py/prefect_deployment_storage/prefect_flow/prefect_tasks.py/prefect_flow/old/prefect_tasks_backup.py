...
async def process_last_message(rabbit_utils: AsyncRabbitMQUtilities, logger: Optional[logging.Logger] = None):
    ...
    try:
        ...
        while (datetime.now() - start_time).total_seconds() < MAX_RUNTIME:
            try:
                messages = await rabbit_utils.fetch_messages (RABBITMQ_CBOE_QUEUE)
...
@task(retries=2)
async def update_book_with_latest_greeks(book: pd.DataFrame, db_utils: AsyncDatabaseUtilities) -> pd.DataFrame:
