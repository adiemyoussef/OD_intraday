...
async def process_last_message_in_queue(rabbitmq_utils:RabbitMQUtilities, expected_file_override = None):
    ...
    try:
        ...
        if expected_file_override:
            ...
            return None, expected_file_name,
        else:
...
@task(retries=2)
def get_file_from_sftp_(msg_body, override_path = None):
    prefect_logger = get_run_logger()
...
def get_file_from_sftp(msg_body, override_path = None):
    prefect_logger = get_run_logger()
    file_path = override_path if override_path else msg_body["path"]
    prefect_logger.info(f"{'[OVERRIDE]' if override_path else '[NO OVERRIDE]'}: Getting {file_path}")
    ...
    try:
        ...
        file_info = sftp_utils.get_file_info(file_path)
        file_data = sftp_utils.read_file(file_path)
        prefect_logger.info(f'It took {time_module.time() - read_time} sec. to read the file')

        return file_info, file_data, file_info['mtime']

    except Exception as e:
        prefect_logger.error(f"Error verifying and processing file {file_path}: {str(e)}")
        raise
    finally:
        sftp_utils.disconnect()
@task(retries=2)
...
def update_book_with_latest_greeks(book: pd.DataFrame, poly_historical_data: pd.DataFrame) -> pd.DataFrame:
    ...
    if latest_poly.empty:
        prefect_logger.warning("latest_poly is empty. Proceeding with historical data only.")
    ...
    if unmerged_contracts > 0:
        ...
        if poly_historical_data.empty:
            ...
            #TODO: Added
            mask = merged_book['iv_update'].isna() if 'iv_update' in merged_book.columns else pd.Series(True,
                                                                                                        index=merged_book.index)
    ...
    # After all updates, identify contracts that weren't updated
    greek_columns = ['iv', 'delta', 'gamma', 'vega']
    update_columns = [f'{greek}_update' for greek in greek_columns if f'{greek}_update' in merged_book.columns]
    historical_columns = [f'{greek}_historical' for greek in greek_columns if
                          f'{greek}_historical' in merged_book.columns]

    not_updated_mask = merged_book[update_columns + historical_columns].isna().all(axis=1)
    not_updated_contracts = merged_book[not_updated_mask]
...
def Intraday_Flow():
    ...
    overrides = ['Cboe_OpenClose_2024-09-19_15_00_1.csv.zip','Cboe_OpenClose_2024-09-19_15_10_1.csv.zip', 'Cboe_OpenClose_2024-09-19_15_20_1.csv.zip', 'Cboe_OpenClose_2024-09-19_15_30_1.csv.zip', 'Cboe_OpenClose_2024-09-19_15_40_1.csv.zip',
                 ...
                 # 'Cboe_OpenClose_2024-09-19_18_00_1.csv.zip']

    #expected_file_override = '/subscriptions/order_000059435/item_000068201/Cboe_OpenClose_2024-09-19_15_10_1.csv.zip' #None
    ...
    try:
        ...
        for override in overrides:
            ...
            previous_date, current_date, previous_datetime, current_datetime = prepare_to_fetch_historical_poly(shift_previous_minutes=60 * 8, shift_current_minutes=0)
            ...
            # message, msg_body = process_last_message_in_queue(rabbitmq_utils, expected_file_override=expected_file_override)
            # breakpoint()
            ...
            try:
                #sftp_utils.ensure_connection()  # Ensure connection before SFTP operation
                if expected_file_override and isinstance(expected_file_override, str):
                    prefect_logger.info(f"Override detected. Jumping directly to file download: {expected_file_override}")
                    file_info, file_data, file_last_modified = get_file_from_sftp({"path": expected_file_override},
                                                                                  expected_file_override)
                    # breakpoint()
                else:

                    message, msg_body = process_last_message_in_queue(rabbitmq_utils,
                                                                      expected_file_override=expected_file_override)
                    message_frame, message_properties, _ = message
                    ...
                    if message is None and msg_body is None:
                        prefect_logger.debug("No messages to process in the queue. Ending the flow.")
                        return
