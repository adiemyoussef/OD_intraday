...
class RabbitMQUtilities:
    ...
    def safe_ack(self, delivery_tag, msg_body):
        try:
            self.ensure_connection()
            self.channel.basic_ack(delivery_tag=delivery_tag)
            self.logger.info(f"Message {msg_body['filename']} acknowledged successfully.")
        except Exception as e:
            self.logger.error(f"Error occurred while acknowledging message {msg_body['filename']}: {e}")
            self.status = RabbitMQStatus.ERROR
            self.last_error = e
            self.reconnect()
            try:
                ...
                self.logger.error(f"Failed to acknowledge message {msg_body['filename']} after reconnection: {inner_e}")
                raise
